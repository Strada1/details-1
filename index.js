import { ELEMENT, maxMessages, POPUP, URL, USER } from "./const.js";
import { showPopup, closePopup, addMessage, showChat, scrollMessagesToEnd, addMessageWebSocket } from "./view.js";
...
function renderMessages() {
	...
	result
		...
		.then(messages => {
			...
			let max = maxMessages;
			for (i; i <= max; i++) {
				addMessage(messages[i].text, messages[i].user.name, messages[i].createdAt, messages[i].user.email);
				scrollMessagesToEnd();
			}
			max += maxMessages;
			ELEMENT.MESSAGES.addEventListener("scroll", () => {
				if (ELEMENT.MESSAGES.scrollTop === 0) {
					let lastMessage = document.querySelector(".message");
					for (i; i <= max; i++) {
						addMessage(messages[i].text, messages[i].user.name, messages[i].createdAt, messages[i].user.name);
					}
					max += maxMessages;
					ELEMENT.MESSAGES.scrollTop = lastMessage.getBoundingClientRect().y;
				}
			});
		});
}
...
function socket() {
	const socket = new WebSocket(`${URL.WEB_SOCKET}${document.cookie}`);

	socket.onmessage = function (event) {
		const data = JSON.parse(event.data);
		addMessageWebSocket(data.text, data.user.name, data.createdAt, data.user.email);
	};
