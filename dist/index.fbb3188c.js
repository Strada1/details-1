...
try {
    ...
    (0, _viewJs.ELEMENTS).taskFormLow.addEventListener("submit", (event)=>addTask(event, (0, _viewJs.PRIORITY).LOW));
} catch (error) {
    (0, _viewJs.ERROR_LIST).ERROR_LISTENER_NOT_FOUND(error);
}
(0, _viewJs.ELEMENTS).themeToggle.addEventListener("click", (event)=>handlerThemeToggle(event));
...
function addDarkTheme() {
    if (localStorage.getItem("theme") === "dark") {
        ...
        (0, _viewJs.ELEMENTS).iconThemeToggle.textContent = "dark_mode";
    } else {
        document.querySelector("html").classList.remove("dark");
        (0, _viewJs.ELEMENTS).iconThemeToggle.textContent = "wb_sunny";
    }
}
showTasks();
addDarkTheme();
...
function showTasks() {
    (0, _storageJs.getFromStorage)();
    // очищает оба High и Low поля
    clearParentDOM((0, _viewJs.ELEMENTS).taskDivHigh);
    clearParentDOM((0, _viewJs.ELEMENTS).taskDivLow);
    if (0, _storageJs.tasks) (0, _storageJs.tasks).forEach((task)=>{
        switch(task.priority){
            case (0, _viewJs.PRIORITY).HIGH:
                (0, _viewJs.ELEMENTS).taskDiv = publishTask(task);
                (0, _viewJs.ELEMENTS).taskDivHigh.prepend((0, _viewJs.ELEMENTS).taskDiv);
                break;
            case (0, _viewJs.PRIORITY).LOW:
                (0, _viewJs.ELEMENTS).taskDiv = publishTask(task);
                (0, _viewJs.ELEMENTS).taskDivLow.prepend((0, _viewJs.ELEMENTS).taskDiv);
                break;
        }
    });
}
...
function publishTask(task) {
    ...
    const handlerDelete = (event)=>{
        event.preventDefault();
        const idDel = task.id;
        const tasksFiltered = (0, _storageJs.tasks).filter((task)=>task.id !== idDel);
        (0, _storageJs.recordToStorage)(tasksFiltered);
        showTasks();
    };
    (0, _viewJs.ELEMENTS).taskDiv = document.createElement("div");
    (0, _viewJs.ELEMENTS).taskDiv.classList = task.status === (0, _viewJs.STATUS).DONE ? "task task-done" : "task";
    // текст задачи
    ...
    (0, _viewJs.DATE).START = (0, _dateFns.format)(new Date(task.id), "hh:mm eee dd MMM");
    dateStart.textContent = `Start: ${(0, _viewJs.DATE).START}`;
    ...
    taskDoneBtn.classList = task.status === (0, _viewJs.STATUS).DONE ? "btn-done checked" : "btn-done";
    taskDoneBtn.addEventListener("click", handlerAddDone);
    (0, _viewJs.ELEMENTS).taskDiv.prepend(taskDoneBtn);
    ...
    (0, _viewJs.ELEMENTS).taskDiv.append(taskDeleteBtn);
    return (0, _viewJs.ELEMENTS).taskDiv;
}
function createNewTask(priority) {
    ...
    switch(priority){
        case (0, _viewJs.PRIORITY).HIGH:
            newTask.name = (0, _viewJs.ELEMENTS).taskInputHigth.value;
            (0, _viewJs.ELEMENTS).taskInputHigth.value = "";
            break;
        case (0, _viewJs.PRIORITY).LOW:
            newTask.name = (0, _viewJs.ELEMENTS).taskInputLow.value;
            (0, _viewJs.ELEMENTS).taskInputLow.value = "";
            break;
    }
    newTask.status = (0, _viewJs.STATUS).TO_DO;
    newTask.priority = priority;
}
function addTask(event, priority) {
    event.preventDefault();
    if ((0, _viewJs.ELEMENTS).taskInputHigth.value.trim() === "" && (0, _viewJs.ELEMENTS).taskInputLow.value.trim() === "") {
        (0, _viewJs.ERROR_LIST).ERROR_EMPTY_ENTER();
        return;
    }
    const newTask = {};
    newTask.id = new Date();
    ...
    (0, _storageJs.tasks).push(newTask);
    (0, _storageJs.recordToStorage)((0, _storageJs.tasks));
    showTasks();
}
