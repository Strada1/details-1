...
function checkFutureDate(futureDate, currentDate) {
    if (currentDate > futureDate) return true;
}
...
function render(duration) {
    ...
    for(item in duration){
        if (duration[item] === 0) continue;
        else if (duration[item] === 1) {
            const timeBlock = createTimeBlock(TIME_UNITS[item].singular, duration[item]);
            results.append(timeBlock);
            continue;
        }
        const timeBlock1 = createTimeBlock(TIME_UNITS[item].plural, duration[item]);
        results.append(timeBlock1);
    }
}
function clearResults() {
    if (document.querySelector(".results") !== null) document.querySelector(".results").remove();
}
...
},{"../_lib/requiredArgs/index.js":"9wUgQ","../constants/index.js":"iOhcx","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["awEvQ","bB7Pu"], "bB7Pu", "parcelRequire4482")
