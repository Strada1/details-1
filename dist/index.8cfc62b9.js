...
function openTab(tabName) {
    ...
    for(i1 = 0; i1 < tabcontent.length; i1++)tabcontent[i1].style.display = "none";
    tablinks = document.getElementsByClassName("tablinks");
    for(i1 = 0; i1 < tablinks.length; i1++)tablinks[i1].className = tablinks[i1].className.replace(" active", "");
    ...
    try {
        ...
        if (!response.ok) inputCity.value = "Ошибка HTTP: " + response.status;
            ...
            if (json.cod === "404") inputCity.value = "City not found ";
                ...
                detailsSunrise.innerHTML = (0, _dateFns.format)(new Date(json.sys.sunrise * 1000), "HH:mm:ss");
                detailsSunset.innerHTML = (0, _dateFns.format)(new Date(json.sys.sunset * 1000), "HH:mm:ss");
            }
        }
    } catch (err) {
...
function refreshAddedLocation() {
    ...
    сitySet.forEach((value)=>{
        let li = `<li class="container-item flex-row addedLocation">
            ...
            <img class="close" src=${(0, _closeSvgDefault.default)} alt="close" cityName="${value}"></li>`;
        addedLocations.insertAdjacentHTML("afterbegin", li);
    });
    let chooseElements = document.getElementsByClassName("container-item-city");
    for(i = 0; i < chooseElements.length; i++)chooseElements[i].addEventListener("click", chooseClick);
    let closeElements = document.getElementsByClassName("close");
    for(i = 0; i < closeElements.length; i++)closeElements[i].addEventListener("click", deleteCityClick);
}
...
},{"./img/close.svg":"ln7uQ","@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3","date-fns":"9yHCA"}],"ln7uQ":[function(require,module,exports) {
module.exports = require("./helpers/bundle-url").getBundleURL("fqV6O") + "close.996aed91.svg" + "?" + Date.now();
...
},{"@parcel/transformer-js/src/esmodule-helpers.js":"gkKU3"}]},["2mNKm","6rimH"], "6rimH", "parcelRequire95a1")
