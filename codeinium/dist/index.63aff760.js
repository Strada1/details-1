...
async function weatherResponse(cityName) {
    try {
        if (!cityName) throw new NameError("не введен город");
        const serverUrl = (0, _itemsJs.API).SERVERURL + `?q=${cityName}&appid=${(0, _itemsJs.API).KEY}&units=metric`;
        const responce = await fetch(serverUrl);
        const data1 = await responce.json();
        const forecastUrl = (0, _itemsJs.API).FORECAST_URL + `?q=${cityName}&appid=${(0, _itemsJs.API).KEY}&units=metric`;
        const response2 = await fetch(forecastUrl);
        const data2 = await response2.json();
        if (data1.cod === "404" && data2.cod === "404") throw new NameError("город не найден");
        //now and details
        (0, _itemsJs.ELEMENTS).WEATHERICON.src = (0, _itemsJs.API).IMG + `${data1.weather[0].icon}@2x.png`;
        (0, _itemsJs.ELEMENTS).TEMPERATURE.textContent = `${Number(data1.main.temp).toFixed(0)}°`;
        ...
        (0, _itemsJs.ELEMENTS).DETAILS_WEATHER.textContent = (0, _itemsJs.TEMPERATURE_WORDS).WEATHER + `${data1.weather[0].main}`;
        const dateSunrise = (0, _dateFns.format)(new Date(data1.sys.sunrise * 1000), "HH:mm");
        const dateSunset = (0, _dateFns.format)(new Date(data1.sys.sunset * 1000), "HH:mm");
        (0, _itemsJs.ELEMENTS).DETAILS_SUNRISE.textContent = (0, _itemsJs.TEMPERATURE_WORDS).SUNRISE + `${dateSunrise}`;
        (0, _itemsJs.ELEMENTS).DETAILS_SUNSET.textContent = (0, _itemsJs.TEMPERATURE_WORDS).SUNSET + `${dateSunset}`;
        //forecast
        (0, _itemsJs.ELEMENTS).FORECAST_LOCATION.textContent = data1.name;
        let i = 0;
        function recurtionForecast(i) {
            if (i === (0, _itemsJs.ELEMENTS).FORECASTS.length) return;
            else {
                const weatherStatus = (0, _itemsJs.ELEMENTS).FORECASTS[i].querySelector("#weather-status");
                const iconStatus = (0, _itemsJs.ELEMENTS).FORECASTS[i].querySelector('img[alt="weather-status"]');
                const date = data2.list[i].dt * 1000;
                const temperature = (0, _itemsJs.ELEMENTS).FORECASTS[i].querySelector("#temper");
                const feels_like = (0, _itemsJs.ELEMENTS).FORECASTS[i].querySelector("#feels");
                const day = (0, _itemsJs.ELEMENTS).FORECASTS[i].querySelector(".dayy");
...
function getCity() {
    const city = (0, _itemsJs.ELEMENTS).FINDINPUT.value;
    (0, _itemsJs.ELEMENTS).FINDINPUT.value = "";
    return city;
}
function addLocationToNow(value) {
    if (!favoriteCities.has(value)) return;
...
function clickHandler() {
    favoriteCities.add((0, _itemsJs.ELEMENTS).LOCATION.textContent);
    (0, _localstorageJs.setFavoriteCities)(JSON.stringify([
        ...favoriteCities
    ]));
    render();
}
(0, _itemsJs.ELEMENTS).ADDBUTTON.addEventListener("click", clickHandler);
(0, _itemsJs.ELEMENTS).FORM.addEventListener("submit", submitHandler);
...
function removingCities(deleteFavoriteCities) {
    if (deleteFavoriteCities.length === 0) return;
    else {
        let item = document.querySelector(".item");
        for (let subitem of deleteFavoriteCities)if (subitem === item) {
            subitem.remove();
...
function renderFavoriteCities() {
    if (favoriteCities.size === 0) return;
    else for (let value of favoriteCities){
        ...
        (0, _itemsJs.ELEMENTS).ADDEDLOCATIONS.prepend(element);
        button.id = value;
...
},{}],"kfd8X":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
