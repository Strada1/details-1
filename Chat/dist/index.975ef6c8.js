...
function sendMessage(name, text) {
    ...
    if (name === myName) temp.querySelector("div").classList.add("me");
    else temp.querySelector("div").classList.add("he");
    temp.querySelector(".message").classList.add("sent");
    const hours = (0, _dateFns.getHours)(Date.now());
    const minutes = (0, _dateFns.getMinutes)(Date.now());
...
function getName() {
    new Fetch("https://edu.strada.one/api/user/me", "GET").then((response)=>response.json()).then((result)=>myName = result.name);
}
function getHistory() {
    new Fetch("https://edu.strada.one/api/messages/", "GET").then((response)=>response.json()).then((result)=>{
        const temp = message.content.cloneNode(true);
        ...
        messages.forEach((message)=>{
            ...
            let hours = (0, _dateFns.getHours)(new Date(date));
            let minutes = (0, _dateFns.getMinutes)(new Date(date));
...
sentEmail.addEventListener("submit", (event)=>{
    ...
    new Fetch("https://edu.strada.one/api/user", "POST", {
        ...
        if (result.status >= 300) throw new Error(result.error);
        else {
            auth.classList.remove("active");
...
sentName.addEventListener("submit", (event)=>{
    ...
    new Fetch("https://edu.strada.one/api/user", "PATCH", {
        name: newName
    }).then((response)=>response.json()).then((result)=>myName = result.name);
    getName();
    settings.classList.remove("active");
});
...
},{}],"k3IR8":[function(require,module,exports) {
var parcelHelpers = require("@parcel/transformer-js/src/esmodule-helpers.js");
parcelHelpers.defineInteropFlag(exports);
var _indexJs = require("../toDate/index.js");
var _indexJsDefault = parcelHelpers.interopDefault(_indexJs);
