...
const formSubmitElement = formElement.querySelector(".countdown__submit");
const formResetElement = formElement.querySelector(".countdown__reset");
...
const clearCountdown = ()=>outputInterval({
        years: 0,
        days: 0,
        hours: 0,
        minutes: 0,
...
const setCountdown = (date)=>{
    setInterval(()=>{
        const interval = (0, _dateFns.intervalToDuration)({
            start: new Date(date),
            end: new Date()
        });
...
const onFormSubmit = (e)=>{
    ...
    if (dateValue) setCountdown(dateValue);
};
formElement.addEventListener("submit", onFormSubmit);
formElement.addEventListener("submit", onFormReset);
