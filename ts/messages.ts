...
export function addMessage(message: messageElements): void {
  ...
  if (message.text.trim() !== "" && ELEMENTS.template) {
    ...
    if (text && time) {
      ...
      time.textContent = format(Date.parse(message.time), "HH:mm");
    }
...
export function downloadHistory(key: string) {
  ...
  const history = messagesList.filter(
    (item: any, index: number) => index >= MESSAGE.step
  );
