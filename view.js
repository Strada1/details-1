...
export function addMessage(messageText, name = USER.ME.NAME, time = "") {
    time = getTime(time);
    if (messageText.trim() === "") {
        return;
    }

    const messageBlock = createMessage(messageText, name, time);
    ELEMENT.MESSAGE_INPUT.textContent = "";
    ELEMENT.MESSAGES.append(messageBlock);
    scrollMessagesToEnd();
}

function createMessage(messageText, name, time) {
    let messageBlock;
    if (name !== USER.ME.NAME) {
